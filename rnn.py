...
def parse_arguments():
    ...
    parser.add_argument("--hidden_dim", default=300, type=int, help="Dimension of each Hidden Layer")
    ...
    parser.add_argument("--num_layers", default=5, type=int, help="Number of repetitive layers in LSTM")
    parser.add_argument("--max_seq_len", default=128, type=int, help="Maximum sequence length of input to the LSTM")
...
# Model
# class LSTMNeuralLM(nn.Module):
#     def __init__(self, vocab_size: int, embedding_dim: int, hidden_dim: int, num_layers: int, dropout_rate=0.1):
#         super(LSTMNeuralLM, self).__init__()
...
#         # We only need the last output for each sequence
#         lstm_out = lstm_out[:, -1, :]
#         # lstm_out shape: (batch_size, hidden_dim)
        
#         lstm_out = self.dropout(lstm_out)
#         fc_out = self.fc(lstm_out)
#         # fc_out shape: (batch_size, vocab_size)
...
def train(args, epoch: int, model: nn.Module, train_loader: DataLoader, optimizer: optim.Optimizer, criterion: nn.Module, device: torch.device, val_loader: DataLoader, scheduler: optim.lr_scheduler=None):
    ...
    for batch_idx, (input_seq, target_seq) in enumerate(train_loader):
        # print(input_seq.shape, target_seq.shape) # (batch_size, seq_len), (batch_size, seq_len)
...
def main():
    ...
    # EPOCHS = 10
    # LEARNING_RATE = 0.001
    # GLOVE_FILE = "GloVe/glove.6B/glove.6B.300d.txt"a
    # CORPUS_PATH = "Auguste_Maquet.txt"
    # TEST_SIZE = 20000
    # VALID_SIZE = 10000
    ...
    model = LSTMNeuralLM(len(vocab), args.embedding_dim, args.hidden_dim, args.num_layers, args.dropout_rate).to(device)
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=vocab['<PAD>']) # ignore padding token from loss calculation steps
